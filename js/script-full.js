...
$(document).ready(function () {
    ...
    footnav = $('nav.footnav');
    footnav.css({
        width: windw.width() - 74
    });
    logo.find('a').bind({
        'click': function (e) {
            e.preventDefault();
            resetfootnav();
            footnav.find('li').eq(0).addClass('active');
            showHome()
        }
    });
    footnav.find('li').each(function (i) {
        var nav = $(this);
        nav.bind({
            'click': function (e) {
                e.preventDefault();
    ...
    about = $('#about');
    about.css({
        left: (windw.width() - 900) / 2,
        top: (windw.height() - 533) / 2
    });
    works = $('#works');
    works.css({
        left: (windw.width() - 900) / 2,
    ...
    actwork = 0;
    worksarr = [];
    works.find('div.work').each(function (index) {
        var work = $(this);
        if (index > 0) work.css({
            opacity: 0,
            display: 'none'
    ...
    prevwork.bind({
        'click': function (e) {
            var c = worksarr.length;
            var a = actwork;
            actwork = a - 1 > -1 ? --a : c - 1;
            worksarr[actwork].css({
                display: 'block',
                opacity: 0
            }).stop(true).animate({
                opacity: 1
    ...
    tags = ['threejs', 'html5', 'jquery', 'javascript'];
    if (Detector.webgl) {
        init();
        animate();
